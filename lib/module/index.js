...
const DivaMobileZebraScanner = NativeModules.DivaMobileZebraScanner ? NativeModules.DivaMobileZebraScanner : new Proxy({}, {
  get() {
...
DivaMobileZebraScanner.startReader = handler => {
  if (!isListenerAssigned) {
    ...
    return DivaMobileZebraScanner.init().then(() => {
      ...
      if (DeviceEventEmitter.addListener(DivaMobileZebraScanner.BARCODE_READ_SUCCESS,
...
DivaMobileZebraScanner.stopReader = () => {
  if (isListenerAssigned) {
    ...
    return DivaMobileZebraScanner.finalize().then(() => {
      ...
      DeviceEventEmitter.removeAllListeners(DivaMobileZebraScanner.BARCODE_READ_SUCCESS);
