...
const ReactNativeZebraScanner = NativeModules.ReactNativeZebraScanner ? NativeModules.ReactNativeZebraScanner : new Proxy({}, {
  get() {
...
ReactNativeZebraScanner.startReader = handler => {
  if (!isListenerAssigned) {
    ...
    return ReactNativeZebraScanner.init().then(() => {
      ...
      if (DeviceEventEmitter.addListener(ReactNativeZebraScanner.BARCODE_READ_SUCCESS,
...
ReactNativeZebraScanner.stopReader = () => {
  if (isListenerAssigned) {
    ...
    return ReactNativeZebraScanner.finalize().then(() => {
      ...
      DeviceEventEmitter.removeAllListeners(ReactNativeZebraScanner.BARCODE_READ_SUCCESS);
